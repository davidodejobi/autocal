...
class TextParserService {
  ...
  static final List<_DatePattern> _datePatterns = [
    ...
    _DatePattern(
      ...
      (match) => _tryParseDate(
        int.parse(match.group(3)!),
        _monthNameToNumber(match.group(1)!),
        int.parse(match.group(2)!),
      ),
      0.95,
    ),
    ...
    _DatePattern(
      ...
      (match) => _tryParseDate(
        int.parse(match.group(3)!),
        _monthNameToNumber(match.group(2)!),
        int.parse(match.group(1)!),
      ),
      0.95,
    ),
  ...
  bool _isOnlyCommonWords(String text) {
    ...
    return meaningfulWords.isEmpty ||
        meaningfulWords.length < words.length * 0.3;
  }
  ...
  static final List<_LocationPattern> _locationPatterns = [
    ...
    _LocationPattern(
      RegExp(
        r'\b(?:room|conference room|meeting room)\s+([A-Za-z0-9\-]+)\b',
        caseSensitive: false,
      ),
    ...
    _LocationPattern(
      RegExp(
        r'\b([A-Za-z\s]+(?:Building|Bldg))\s*,?\s*(?:room|floor|level)?\s*([A-Za-z0-9\-]+)?\b',
        caseSensitive: false,
      ),
      (match) {
    ...
    _LocationPattern(
      ...
      (match) =>
          '${match.group(1)!.trim()}, ${match.group(2)} ${match.group(3)}',
      0.9,
    ),
    // City, State format
    _LocationPattern(
    ...
    _LocationPattern(
      RegExp(
        r'\b(zoom|teams|skype|webex|google meet|hangouts|online|virtual|remote)\b',
        caseSensitive: false,
      ),
    ...
    for (final pattern in _locationPatterns) {
      ...
      for (final match in regexMatches) {
        ...
        if (location.isNotEmpty && _isValidLocation(location)) {
          matches.add(
            _LocationMatch(location, pattern.confidence, match.group(0)!),
          );
        }
      }
    }
  ...
  List<String> extractLocations(String text) {
    return extractLocationsWithConfidence(
      text,
    ).map((match) => match.location).toList();
  }
  ...
  static List<_LocationMatch> _removeDuplicateLocations(
    List<_LocationMatch> matches,
  ) {
